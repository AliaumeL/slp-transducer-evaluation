...
class SLP(Generic[I]):
    ...
    def run_SST_output_slp(self, sst: SST[I, O, S, R], EMPTY:O) -> 'SLP[O]':
        ...
        initial_slp_regs : dict[R, SLP] = {r : (self.string_to_slp_naive(sst.init_regs[r])) for r in sst.registers}

        for c in self.constants:
            slp_regs.append({(q, r) : (self.string_to_slp_naive(sst.reg_update[q, c][r])) for q in sst.states for r in sst.registers})
        ...
        for i, j in self.instructions:
            ...
            print(f"DEBUG: processing instruction {i, j}")
                ...
                for r in sst.registers:
                    ...
                    for reg in regs_list:
                            ...
                            if (len(slp_i.instructions) == 0): # reg update is only a constant
                                reg_to_instrn[reg] = const_to_idx[slp_i.constants[-1]] # check
                                continue
                    ...
                    composed_slpreg[q, r] = composed_slp # check
        ...
        for token in final_output:
            if token in sst.registers:
                ...
                if (found_regs[token]): #type: ignore
                    indices_queue.append(reg_roots[token]) #type: ignore
                    continue

                found_regs[token] = True # type: ignore

                reg_slp = reg_map[(sst.init_state, token)]  # type: ignore
                clen = len(reg_slp.constants)

                if (len(reg_slp.instructions) == 0 and clen == 0): # reg update is empty
                    reg_roots[token] = 0 #type: ignore
                    continue

                if (len(reg_slp.instructions) == 0):
                    reg_roots[token] = const_to_idx[reg_slp.constants[-1]] #type: ignore
                    continue                
                ...
                reg_roots[token] = reg_root # type: ignore
    ...
    @staticmethod
    def string_to_slp_naive(w : list[Union[R, O]]) -> 'SLP[Union[R, O]]':
        ...
        const_index: dict[Union[R, O], int] = {c: i for i, c in enumerate(constants)} #type:ignore

        instructions: list[tuple[int, int]] = []
        for i in range(1, len(w)):
            if (i == 1):
                instructions.append((const_index[w[i - 1]], const_index[w[i]])) #type:ignore
            else:
                instructions.append((i + len(constants) - 2, const_index[w[i]])) #type:ignore

        return SLP(constants=constants, instructions=instructions) #type:ignore
    

# Previous Iteration

    # def run_SST_output_slp(self, sst: SST[I, O, S, R], EMPTY : O) -> 'SLP[O]':
    #     """ Generates SLPs as input is processed for output compression to avoid blowup in case of exponential length outputs"""

    #     print(f"DEBUG: Input SLP constants: {self.constants}")
    #     print(f"DEBUG: Input SLP instructions: {self.instructions}")
    #     print(f"DEBUG: SST initial registers: {sst.init_regs}")

        
    #     slp_regs: list[dict[tuple[S, R], SLP]] = [] # slp[q, ri, xi] depends on slps of xj, xk if xi = xj.xk
    #     tr_maps: list[dict[S, S]] = []
    ...
    #     for r in sst.registers:
    #         print(f"INITIAL SLP: {initial_slp_regs[r]}")

    #     for c in self.constants:
    #         print(f"DEBUG: Processing constant '{c}'")
    #         # slp_regs tracks state x reg x input -> new reg expression compressed as SLP
    #         slp_regs.append({(q, r) : (self.string_to_slp_naive(sst.reg_update[q, c][r])) for q in sst.states for r in sst.registers})
    #         # state x input -> state
    #         print(f"DEBUG: slp_reg {slp_regs[-1]}")
    #         tr_maps.append({q : sst.delta[(q, c)] for q in sst.states})
    ...
    #             # each reg_update will be linear so we can go through the reg update and determine which all registers are needed for the composed reg_update
    #             for r in sst.registers:
    #                 slp_j = slpreg_j[q_i, r]
    #                 # print(f"DEBUG: slp_j : {slp_j}")
    #                 print(f"Reg {r}: Processing slp_j with constants={slp_j.constants}, instructions={slp_j.instructions}")
                    
    #                 # consider for empty case (r = []), then there wont be any instructions
    #                 if (len(slp_j.instructions) == 0 and len(slp_j.constants) == 0):
    #                     composed_slpreg[q, r] = SLP(constants=[], instructions=[])
    #                     continue

    #                 composed_slp : SLP[Union[R, O]] = SLP(constants=[EMPTY], instructions=[])
    #                 const_counter = 1
    #                 instrn_counter = 0
    #                 reg_to_instrn : dict[R, int] = {} # need instruction co-ordinate to map reg const in slp_j to the respective reg update instruction
    #                 found_regs : dict[R, int] = {r: 0 for r in sst.registers} # to avoid multiple SLP of copies of the same register update
    #                 total_consts : int = len(slp_j.constants) + 1
    #                 found_r : bool = False

    #                 for token in slp_j.constants:
    #                     if (token in sst.registers): # register constant variable found
    #                         if (found_regs[token] == 1):
    #                             continue

    #                         found_regs[token] = 1
    #                         found_r = True    
    #                         slp_i = slpreg_i[(q, token)]
    #                         total_consts += len(slp_i.constants)

    #                 for token in slp_j.constants:
    #                     if (token in sst.registers): # register constant variable found
    #                         print(f"DEUBG: found register = {token}")
    #                         if (found_regs[token] == 2):
    #                             continue

    #                         found_regs[token] = 2

    #                         slp_i = slpreg_i[(q, token)]
    #                         print(f"Found reg's slp_i : {slp_i}")
    #                         composed_slp.constants.extend(slp_i.constants)
    #                         clen : int = len(slp_i.constants)

    #                         if (len(slp_i.instructions) == 0 and clen == 0): # reg update is empty
    #                             reg_to_instrn[token] = 0 # point to EMPTY # will always be correct
    #                             continue

    #                         if (len(slp_i.instructions) == 0): # reg update is only a constant
    #                             const_counter += len(slp_i.constants)
    #                             reg_to_instrn[token] = const_counter - 1
    #                             continue

    #                         for x, y in slp_i.instructions:
    #                             print(f"instruction ({x, y})")
    #                             x_new = (x + const_counter) if (x < clen) else (x + instrn_counter + total_consts - (len(slp_i.constants)))
    #                             y_new = (y + const_counter) if (y < clen) else (y + instrn_counter + total_consts - (len(slp_i.constants)))
    #                             composed_slp.instructions.append((x_new, y_new))

    #                         const_counter += len(slp_i.constants)
    #                         instrn_counter += len(slp_i.instructions)

    #                         reg_to_instrn[token] = instrn_counter + total_consts - 1
    #                         print(f"After substitution: reg_to_instrn[{token}] = {reg_to_instrn[token]}")
                    
    #                 if (found_r is False): # reg_update does not use previous register values 
    #                     composed_slp = SLP(constants=slp_j.constants[:], instructions=slp_j.instructions[:])
    #                     print(f"COMPOSED SLP: {composed_slp}")

    #                 elif (len(slp_j.instructions) != 0):
    #                     # jth slp itself must be added to composed slp
    #                     # if it has reached to this part of the loop, then slp_j has instructions
    #                     composed_slp.constants.extend(slp_j.constants) 
    #                     clen_j : int = len(slp_j.constants)

    #                     for x, y in slp_j.instructions:
    #                         # 3 cases : x refers to reg constant, normal constant or a previous intruction
    #                         x_new = (reg_to_instrn[slp_j.constants[x]]) if (x < clen_j and slp_j.constants[x] in reg_to_instrn) else (x + const_counter) if (x < clen_j) else (x + instrn_counter + total_consts - len(slp_j.constants))
    #                         y_new = (reg_to_instrn[slp_j.constants[y]]) if (y < clen_j and slp_j.constants[y] in reg_to_instrn) else (y + const_counter) if (y < clen_j) else (y + instrn_counter + total_consts - len(slp_j.constants))
    #                         composed_slp.instructions.append((x_new, y_new))   
                    
    #                 composed_slpreg[q, r] = SLP(constants = composed_slp.constants[:], instructions = composed_slp.instructions[:])
    #                 print(f"COMPOSED SLP: {composed_slp}")
    ...
    #     output_slp = SLP(constants=[EMPTY], instructions=[])
    #     const_counter = 1
    #     instrn_counter = 0
    #     total_consts = 1

    #     for r, slp_init in initial_slp_regs.items():
    #         total_consts += len(slp_init.constants)

    #     found_regs : dict[R, int] = {r: 0 for r in sst.registers}

    #     for token in final_output:
    #         if token in sst.registers:
    #             # multiple possible as output function need not be copyless, avoid duplicate imports
    #             if (found_regs[token] == 1): #type: ignore
    #                 continue
    #             found_regs[token] = 1 #type: ignore
    #             reg_slp = reg_map[(sst.init_state, token)]  # type: ignore
    #             total_consts += len(reg_slp.constants)
    #         else:
    #             total_consts += 1 # just a constant
    ...
    #     for r, slp_init in initial_slp_regs.items():
    #         clen = len(slp_init.constants)
    #         output_slp.constants.extend(slp_init.constants)
    ...
    #         if (len(slp_init.instructions) == 0):
    #             const_counter += clen
    #             reg_to_initval[r] = const_counter - 1
    #             continue

    #         for x, y in slp_init.instructions:
    #             x_new = x + const_counter if x < clen else x + instrn_counter + total_consts - len(slp_init.constants)
    #             y_new = y + const_counter if y < clen else y + instrn_counter + total_consts - len(slp_init.constants)
    #             output_slp.instructions.append((x_new, y_new))

    #         const_counter += len(slp_init.constants)
    #         instrn_counter += len(slp_init.instructions)
    #         reg_to_initval[r] = total_consts + instrn_counter - 1
    ...
    #         if token in sst.registers:
    #             # multiple possible as output function need not be copyless, avoid duplicate imports
    #             if (found_regs[token] == 2): #type: ignore
    #                 indices_queue.append(reg_roots[token]) #type: ignore
    #                 continue

    #             found_regs[token] = 2 # type: ignore

    #             reg_slp = reg_map[(sst.init_state, token)]  # type: ignore
    #             clen = len(reg_slp.constants)
    #             output_slp.constants.extend(reg_slp.constants)
    ...
    #             if (len(reg_slp.instructions) == 0):
    #                 const_counter += clen
    #                 reg_roots[token] = const_counter - 1 #type: ignore
    #                 continue                

    #             for x, y in reg_slp.instructions:
    #                 x_new = reg_to_initval[reg_slp.constants[x]] if (x < clen and reg_slp.constants[x] in sst.registers) else x + const_counter if (x < clen) else x + instrn_counter + total_consts - len(reg_slp.constants)
    #                 y_new = reg_to_initval[reg_slp.constants[y]] if (y < clen and reg_slp.constants[y] in sst.registers) else y + const_counter if (y < clen) else y + instrn_counter + total_consts - len(reg_slp.constants)
    #                 output_slp.instructions.append((x_new, y_new))

    #             const_counter += clen
    #             instrn_counter += len(reg_slp.instructions)
    #             reg_root = total_consts + instrn_counter - 1
    #             reg_roots[token] = reg_root # type: ignore
    #             indices_queue.append(reg_root)

    #         else:
    #             # It's a constant output symbol
    #             output_slp.constants.append(token) #type: ignore
    #             indices_queue.append(const_counter)
    #             const_counter += 1
    ...
    #     else: # length is 1
    #         a = indices_queue.pop(0)
    #         # if (a < len(output_slp.constants)):
    #         #     return SLP(constants=[output_slp.constants[a]], instructions=[])
    #         output_slp.instructions.append((0, a))             
